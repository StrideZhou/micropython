...
function ci_unix_qemu_aarch64_run_tests {
    ...
    # - cmdline/repl_autoindent.py depends on REPL echo timing, which is much
    #   slower under user-mode emulation (excluded here rather than globally in
    #   run-tests.py, so the other CI jobs keep their coverage)
    local exclude='thread/stress_aes.py|thread/stress_recurse.py|thread/thread_gc1.py|cmdline/repl_autoindent.py'
    file ./ports/unix/build-coverage/micropython
    ulimit -n 1024
    wrapper=$(pwd)/ports/unix/build-coverage/micropython-qemu.sh
